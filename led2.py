...
import time
import threading
import requests  # Importar requests para realizar peticiones HTTP
...
def say_greeting(name: str) -> None:
    ...
    requests.get("http://192.168.0.6:6500/led/on")  # Reemplaza con la IP correcta de tu Raspberry Pi
    ...
    requests.get("http://192.168.0.6:6500/led/off")  # Reemplaza con la IP correcta de tu Raspberry Pi
    ...
    while True:
        ...
        if current_recognized_name != last_recognized_name and current_recognized_name != "Unknown":
            if last_recognized_name != "Unknown":
                # Restablecer el saludo para la persona anterior
                greeting_active[last_recognized_name] = False
